...
public abstract class RecipeMatch {
    ...
    public abstract Optional<Match> matches(DefaultedList<ItemStack> stacks);
     ...
     */
    public static void removeMatch(DefaultedList<ItemStack> stacks, Match match) {
    ...
    private static void removeOrder(DefaultedList<ItemStack> stacks, List<ItemStack> toRemove, int[] amountsRemoved) {
        int i = 0;
        for (int j = 0; j < amountsRemoved.length; j++) {
            ItemStack stack = toRemove.get(j);
            for (; i < stacks.size(); i++) {
                // nbt sensitive since toolparts etc. use nbt
                if (ItemStack.areItemsEqual(stack, stacks.get(i)) && ItemStack.areTagsEqual(stack, stacks.get(i))) {
                    if (stacks.get(i).getCount() >= stack.getCount()) {
                        stacks.get(i).decrement(stack.getCount());
    ...
    private static void removeRemaining(DefaultedList<ItemStack> stacks, List<ItemStack> toRemove, int[] amountsRemoved) {
        for (int j = 0; j < amountsRemoved.length; j++) {
            ItemStack stack = toRemove.get(j);
        ...
        public Optional<Match> matches(DefaultedList<ItemStack> stacks) {
            ...
            for (ItemStack stack : stacks) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (ItemStack.areItemsEqual(template, stack) && ItemStack.areTagsEqual(template, stack)) {
                        // add the amount found to the list
                        ItemStack copy = stack.copy();
                        copy.setCount(1);
                        found.add(copy);
