...
public class SectionDataModule extends SectionData {
    ...
    protected ArrayList<PageData> getPages(String data) {
        ...
        if (this.source instanceof ModuleFileRepository) {
            ...
            return new ArrayList<>(Arrays.stream(BookLoader.GSON.fromJson(data, PageDataModule[].class)).filter((page) -> page.module.isEmpty() || manager.test(page.module)).collect(Collectors.toList()));
        }
