...
public class BookScreen extends Screen {
    ...
    public BookScreen(Text title, BookData book, @Nullable ItemStack item) {
        ...
        this.font = this.minecraft.textRenderer;
        
        initWidthsAndHeights();
        
        this.advancementCache = new AdvancementCache();
        this.minecraft.player.networkHandler.getAdvancementHandler().setListener(this.advancementCache);
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (fontRenderer == null) {
            fontRenderer = this.minecraft.textRenderer;
        }
        
        if (debug) {
            fill(0, 0, fontRenderer.getStringWidth("DEBUG") + 4, fontRenderer.fontHeight + 4, 0x55000000);
            fontRenderer.draw("DEBUG", 2, 2, 0xFFFFFFFF);
        }
...
*/
        ...
        TextureManager render = this.minecraft.getTextureManager();
        ...
        if (this.page == -1) {
            ...
            if (!this.book.appearance.title.isEmpty()) {
                ...
                fontRenderer.drawWithShadow(this.book.appearance.title, (this.width / 2) / scale + 3 - fontRenderer.getStringWidth(this.book.appearance.title) / 2, (this.height / 2 - fontRenderer.fontHeight / 2) / scale - 4, 0xAE8000);
            ...
            if (!this.book.appearance.subtitle.isEmpty()) {
                ...
                fontRenderer.drawWithShadow(this.book.appearance.subtitle, (this.width / 2) / 1.5F + 7 - fontRenderer.getStringWidth(this.book.appearance.subtitle) / 2, (this.height / 2 + 100 - fontRenderer.fontHeight * 2) / 1.5F, 0xAE8000);
                GlStateManager.popMatrix();
            }
        } else {
            ...
            if (this.page != 0) {
                ...
                if (this.book.appearance.drawPageNumbers) {
                    ...
                    fontRenderer.draw(pNum, PAGE_WIDTH / 2 - fontRenderer.getStringWidth(pNum) / 2, PAGE_HEIGHT - 10, 0xFFAAAAAA);
                }
  ...
  public void actionPerformed(GuiButton button) {
    ...
    protected int getMouseX(boolean rightSide) {
        return (int) ((MinecraftClient.getInstance().mouse.getX() * this.width / this.minecraft.window.getFramebufferWidth() - this.leftOffset(rightSide)) / PAGE_SCALE);
    }
    
    protected int getMouseY() {
        return (int) ((MinecraftClient.getInstance().mouse.getY() * this.height / this.minecraft.window.getFramebufferHeight() - 1 - this.topOffset()) / PAGE_SCALE);
    }
