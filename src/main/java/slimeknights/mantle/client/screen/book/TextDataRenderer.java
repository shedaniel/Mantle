...
public class TextDataRenderer {
     ...
     */
    public static String drawText(int x, int y, int boxWidth, int boxHeight, TextData[] data, int mouseX, int mouseY, TextRenderer fr) {
    ...
    public static String drawText(int x, int y, int boxWidth, int boxHeight, TextData[] data, int mouseX, int mouseY, TextRenderer fr,
        ...
        for (TextData item : data) {
            ...
            boolean b = mouseX >= box1X && mouseX <= box1W && mouseY >= box1Y && mouseY <= box1H && box1X != box1W && box1Y != box1H;
            boolean b1 = b || (mouseX >= box2X && mouseX <= box2W && mouseY >= box2Y && mouseY <= box2H && box2X != box2W && box2Y != box2H) || (mouseX >= box3X && mouseX <= box3W && mouseY >= box3Y && mouseY <= box3H && box3X != box3W && box1Y != box3H);
        ...
        if (BookScreen.debug && action != null && !action.isEmpty()) {
            tooltip.add("");
            tooltip.add(Formatting.GRAY + "Action: " + action);
        }
    ...
    public static void drawTooltip(List<String> textLines, int mouseX, int mouseY) {
        MinecraftClient.getInstance().currentScreen.renderTooltip(textLines, mouseX, mouseY);
        GuiLighting.disable();
    }
    
    public static void drawScaledString(TextRenderer font, String text, float x, float y, int color, boolean dropShadow, float scale) {
        GlStateManager.pushMatrix();
        GlStateManager.translatef(x, y, 0);
        GlStateManager.scalef(scale, scale, 1F);
        if (dropShadow) {
            font.drawWithShadow(text, 0, 0, color);
        } else {
